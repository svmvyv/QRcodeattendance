import 'package:qr_code_scanner_example/screens/SectionLecScreen.dart';
...
import 'dart:convert';
class ViewAttendanceReportScreen extends StatefulWidget {

  _ViewAttendanceReportScreenState createState() => _ViewAttendanceReportScreenState();
}

class _ViewAttendanceReportScreenState extends State<ViewAttendanceReportScreen> {
  List<Courses> course = [];
  Future<List<Courses>> courses;
  var baseUrl = serverUrl + 'Eattendance/Doctor/Doctorsectios.php';
  var isLoading = false;
   _fetchData() async {
    setState(() {
      isLoading = true;
        ...
        subtitle: Column(
          children: <Widget>[
            Text(ccourse.sectiondays +
                '(' +
                ccourse.StartTime +  ccourse.EndTime +
                ')'),
            TextButton.icon(
                label: Text('Section Lectures'),
                icon: Icon(Icons.web),
                style: TextButton.styleFrom(primary: Colors.lightBlue),
                onPressed: () {
                  var route = new MaterialPageRoute(
                    builder: (BuildContext context) => new SectionLecScreen(ccourse.SectionID),
                  );
